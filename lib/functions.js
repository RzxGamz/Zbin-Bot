...
module.exports = class Functions {
  ...
  async metadataMsg(client, msg) {
    let chatMeta = async(mess) => {
      ...
      mess.string = (mess.type === 'conversation') ? mess.message.conversation: (mess.data.includes('caption')) ? mess.message[mess.type].caption: (mess.type == 'extendedTextMessage') ? mess.message[mess.type].text: (mess.type == 'buttonsResponseMessage') ? mess.message[mess.type].selectedButtonId: (mess.type == 'listResponseMessage') ? mess.message[mess.type].singleSelectReply.selectedRowId: ''
      ...
      mess.groupData = mess.isGroup ? {}: false
  ...
  async run() {
    try {
      ...
      global.session = baileys.useSingleFileAuthState('./src/json/session.json')
